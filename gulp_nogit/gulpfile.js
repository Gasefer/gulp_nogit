...
(() => {
    ...
    const gulp = require('gulp'),
        ...
        path = {
            ...
            src: {
                ...
                fonts: 'src/fonts/**/*.*',
                vendor: Object.keys(dependencies).map(dependency => `./node_modules/${dependency}`),
    ...
    function sprite(done) {
        const spriteData = gulp.src(path.src.imgicons)
            .pipe(spritesmith({
                imgName: 'icons.png',
                cssName: 'icons.scss',
    ...
    function smtpBuild(done) {
        const mfilter = filter(['**/*.*']);
        gulp.src('smtp/.env.example')
            .pipe(gulp.dest(path.build.smtp));

        gulp.src(path.src.path_smtp)
            .pipe(mfilter)
            .pipe(gulp.dest(path.build.smtp))
        done();
    }
    ...
    function testBuild(done) {
        ...
        gulp.src(path.build.smtp + '**/*.*')
            .pipe(gulp.dest(path.test.smtp));

        gulp.src(path.build.media + '**/*.*')
            .pipe(gulp.dest(path.test.media));
    ...
    exports.build = gulp.series(cleanDir, imageBuild, sprite, svgSpriteBuild, fontsBuild, vendorBuild, jsBuild, styleBuild, smtpBuild, mediaBuild, htmlBuild);;
    ...
    exports.deploy = (done) => {
        ...
        gulp.src(globs, { base: './' + path, buffer: false })
